...
void VideoCamera::write(Mat& frame)
{
	if (writer == NULL)
	{
		...
		writer = new VideoWriter("out.avi", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 1, Size(frame_width, frame_height), true);
	}
	writer->write(frame);
}
...
void VideoCamera::probeUrls() {
	...
	if (!cap.isOpened()) {
		...
		if (!cap.isOpened()) {
			...
			if (!cap.isOpened()) {
				...
				printf("\nTry to open USB0 ...");
				cap.open(0 + cv::VideoCaptureAPIs::CAP_PVAPI);
				if (!cap.isOpened()) {
					printf("failed\n");
					return;
				}
	...
	printf("cv::VideoCaptureProperties::CAP_PROP_FRAME_WIDTH properties:  %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_FRAME_WIDTH));
	printf("cv::VideoCaptureProperties::CAP_PROP_FRAME_HEIGHT properties: %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_FRAME_HEIGHT));
	printf("cv::VideoCaptureProperties::CAP_PROP_FPS properties:          %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_FPS));
	printf("cv::VideoCaptureProperties::CAP_PROP_FORMAT properties:       %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_FORMAT));
	printf("cv::VideoCaptureProperties::CAP_PROP_MODE properties:         %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_MODE));
	printf("cv::VideoCaptureProperties::CAP_PROP_BRIGHTNESS properties:   %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_BRIGHTNESS));
	printf("cv::VideoCaptureProperties::CAP_PROP_CONTRAST properties:     %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_CONTRAST));
	printf("cv::VideoCaptureProperties::CAP_PROP_SATURATION properties:   %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_SATURATION));
	printf("cv::VideoCaptureProperties::CAP_PROP_HUE properties:          %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_HUE));
	printf("cv::VideoCaptureProperties::CAP_PROP_GAIN properties:         %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_GAIN));
	printf("cv::VideoCaptureProperties::CAP_PROP_EXPOSURE properties:     %f\n",
		cap.get(cv::VideoCaptureProperties::CAP_PROP_EXPOSURE));
	printf("cv::VideoCaptureProperties::CAP_PROP_CONVERT_RGB properties:  %f\n",
